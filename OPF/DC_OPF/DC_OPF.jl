...
function DC_OPF(dLine::DataFrame, dGen::DataFrame, dNodes::Vector{DataFrame}, nN::Int, nL::Int, bMVA::Int, solver::String, hours::Int, dSolar::DataFrame, dWind::DataFrame, dStorage::DataFrame)
    ...
    all_solFlows = DataFrames.DataFrame(hour = Int[], fbus = Int[], tbus = Int[], flow = Float64[], loading=Float64[])  # Power flow through each line by hour
    ...
    storage_results = DataFrames.DataFrame(hour = Int[], bus = Int[], E_i = Float64[], E_s = Float64[], y_s = Int64[])
    ...
    if solver == "Gurobi"
        ...
        set_optimizer_attribute(m, "ResultFile", "model.lp")
        set_optimizer_attribute(m, "ResultFile", "model.mps")
    ...
    @variable(m, P_G[i in 1:nN, t in 1:hours])

    # Assume the voltage magnitude at all nodes is constant (V = 1),
    # and only the angle varies
    @variable(m, θ[i in 1:nN, t in 1:hours], upper_bound = π, lower_bound = -π)
    ...
    @variable(m, P_Curt[i in 1:nN, t in 1:hours])

    # Energy storage variables
    @variable(m, E_s[i in 1:nN, t in 0:hours])

    # Power of charging and discharging of the storage
    @variable(m, P_s_c[i in 1:nN, t in 1:hours])
    @variable(m, P_s_d[i in 1:nN, t in 1:hours])
    ...
    m.ext[:objective]        = OrderedDict{Symbol, Any}() # Objective
    # @objective(m, Min, sum(P_Cost0[i] + P_Cost1[i] * P_G[i,t] * bMVA + P_Cost2[i] * (P_G[i,t] * bMVA)^2 for i in 1:nN, t in 1:hours))

    
    m.ext[:objective] = JuMP.@objective(m, Min, sum(P_Cost1[i] * P_G[i,t] * bMVA for i in 1:nN, t in 1:hours))
    ...
    for k in 1:nL
        if dLine.status[k] != 0
            ...
            # @constraint(m, [t in 1:hours], - 1.5 * rateA_k_t[k,t] / bMVA <= B_k_t[k,t] * (θ[dLine.fbus[k],t] - θ[dLine.tbus[k],t]) <= 1.5 * rateA_k_t[k,t] / bMVA)
            # @constraint(m, [t in 1:hours], -Inf <= B_k_t[k,t] * (θ[dLine.fbus[k],t] - θ[dLine.tbus[k],t]) <= Inf)
        end
    end
    ...
    @constraint(m, [i in 1:nN, t in 1:hours], 0.0 <= P_G[i,t])
    @constraint(m, [i in 1:nN, t in 1:hours], P_G[i,t] <= P_Gen_ub_t[i,t] * Gen_Status_t[i,t])
    ...
    ### "THERMAL" GENERATOR RAMPS ###
    @constraint(m, [i in 1:nN, t in 2:hours], 
        -P_rr[i] <= P_G[i,t] - P_G[i,t-1])
    @constraint(m, [i in 1:nN, t in 2:hours], 
        P_G[i,t] - P_G[i,t-1] <= P_rr[i])
    ...
    # for i in 1:nN
    #     initial_soc = [0 eta_c[i]*P_s_c_max[i]] # Initial state of charge of a battery
    #     # println(initial_soc)
    #     @constraint(m, [i in 1:nN], E_s[i,0] == rand(initial_soc))
    # end

    # for i in 1:nN
    #     @constraint(m, [i in 1:nN], E_s[i,0] == rand(Uniform(0, 1)) * P_s_c_max[i])
    # end
    ...
    if termination_status(m) == OPTIMAL || termination_status(m) == LOCALLY_SOLVED || termination_status(m) == ITERATION_LIMIT
        ...
        for t in 1:hours
            ...
            operation_cost_t = sum(P_Cost1[i] * value(P_G[i, t]) * bMVA for i in 1:nN)
            ...
            solFlows = DataFrames.DataFrame(hour = Int[], fbus = Int[], tbus = Int[], flow = Float64[], loading=Float64[])
            ...
            for i in 1:nL
                ...
                push!(solFlows, Dict(
                    ...
                    :loading => abs(flow_value) / dLine.rateA[i]
                ))
            end
            ...
            solStorage = DataFrames.DataFrame(hour = Int[], bus = Int[], E_i = Float64[], E_s = Float64[], y_s = Int64[])
            for i in 1:nN
    ...
    return m, all_solGen, all_solFlows, all_solVoltage, costs_by_hour, curtailment_results, storage_results
end